...
(function () {
  ...
  // ---------- emotes ----------
  // The in-game crew strip and emote bar were removed for a full-screen board.
  // We still render an emote if one arrives, but the client no longer sends them.
  ...
  // Releasing focus (tab switch) can swallow keyup — stop steering to be safe.
  ...
  // ---------- audio (music master + source, SFX) ----------
  // Two underlying switches drive everything: KSMusic mute = the built-in game
  // soundtrack, KSRadio mute = the crew's YouTube radio. The pause menu exposes
  // them as a master "Music on/off" (both) plus a "Game ↔ YouTube" source pick;
  // the home screen keeps its quick music + SFX icons.
  function setMusicMuted(m) { if (KSMusic.isMuted() !== m) KSMusic.toggleMute(); }
  function setRadioMuted(m) { if (window.KSRadio && KSRadio.isMuted() !== m) KSRadio.toggleMute(); }
  function musicOn() { return !KSMusic.isMuted(); }
  function musicSource() { return (window.KSRadio && !KSRadio.isMuted()) ? 'youtube' : 'game'; }

  function setMusicMaster(on) {
    setMusicMuted(!on);
    if (!on) setRadioMuted(true);   // master off silences the radio too
    applyRadioDucking();            // the soundtrack yields to an audible radio
    updateMuteBtn();
  }
  function setMusicSource(src) {
    if (!musicOn()) setMusicMuted(false);  // picking a source implies music on
    setRadioMuted(src !== 'youtube');
    applyRadioDucking();
    updateMuteBtn();
  }

  function updateMuteBtn() {
    const sfxOff = SFX.isMuted();
    const musOff = !musicOn();
    const src = musicSource();
    // home-screen quick icons
    $('btn-sfx-home').textContent = sfxOff ? '🔇' : '🔊';
    $('btn-sfx-home').classList.toggle('off', sfxOff);
    $('btn-music-home').classList.toggle('off', musOff);
    // pause-menu toggles
    if ($('btn-pm-music')) {
      $('pm-music-state').textContent = musOff ? 'Off' : 'On';
      $('btn-pm-music').classList.toggle('off', musOff);
    }
    if ($('btn-pm-sfx')) {
      $('pm-sfx-state').textContent = sfxOff ? 'Off' : 'On';
      $('btn-pm-sfx').classList.toggle('off', sfxOff);
    }
    if ($('btn-pm-source')) {
      $('pm-source-state').textContent = src === 'youtube' ? 'YouTube' : 'Game';
      $('btn-pm-source').disabled = musOff;  // no source to pick with music off
    }
  }

  $('btn-sfx-home').onclick = () => { SFX.toggleMute(); SFX.unlock(); SFX.tap(); updateMuteBtn(); };
  $('btn-music-home').onclick = () => { setMusicMaster(!musicOn()); SFX.tap(); };
  $('btn-pm-music').onclick = () => { setMusicMaster(!musicOn()); SFX.tap(); };
  $('btn-pm-sfx').onclick = () => { SFX.toggleMute(); SFX.unlock(); SFX.tap(); updateMuteBtn(); };
  $('btn-pm-source').onclick = () => { setMusicSource(musicSource() === 'youtube' ? 'game' : 'youtube'); SFX.tap(); };
  updateMuteBtn();
