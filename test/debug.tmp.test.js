...
test('two players: create, join, play a round, progress persists', async () => {
  ...
  const tyler = await connect(port); tyler.onAny((e)=>console.log('TYLER <-',e));
  const sib = await connect(port); sib.onAny((e)=>console.log('SIB <-',e)); sib.on('connect_error',(e)=>console.log('SIB ERR',e.message));
